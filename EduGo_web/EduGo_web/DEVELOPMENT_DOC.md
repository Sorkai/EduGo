...
智能测评模块（IntelligentAssessment.vue）提供了在线测评功能，包括教师创建测评和学生参与测评两个主要部分。

### 页面结构

智能测评页面分为两个主要标签页：

1. **参与测评**：学生视图，显示可参与的测评列表和测评详情。
2. **管理测评**：教师视图，显示教师创建的测评列表和测评管理功能。

### 教师功能

- 创建测评：设置测评标题和描述
- 添加题目：支持单选题，包括题目内容、选项、正确答案、分值和解析
- 发布测评：设置测评时间范围和参与学生
- 关闭测评：手动结束测评
- 查看学生完成情况：包括完成状态、得分和完成时间

### 学生功能

- 查看可参与的测评列表：显示测评标题、时间范围和状态
- 开始测评：进入测评答题界面
- 在线答题：选择题目答案
- 提交答案：系统自动评分
- 查看测评结果：包括得分、正确答案、解析和AI分析
...
页面使用Vue 3的Composition API进行状态管理，主要包括：

- 用户角色状态：控制页面显示的内容和功能
- 测评列表状态：存储测评列表数据
- 当前测评状态：存储当前查看或操作的测评详情
- 题目列表状态：存储当前测评的题目列表
- 学生答案状态：存储学生的答题情况
- 测评结果状态：存储测评结果和AI分析
...
页面通过Axios与后端API进行交互，主要包括：

- 获取测评列表：根据用户角色获取教师创建的测评或学生可参与的测评
- 获取测评详情：包括测评基本信息和题目列表
- 创建测评：提交测评标题和描述
- 添加题目：提交题目内容、选项、答案、分值和解析
- 发布测评：提交测评时间范围和参与学生
- 关闭测评：更新测评状态
- 开始测评：更新学生测评状态
- 提交答案：提交学生答案并获取评分结果
- 获取测评结果：获取学生测评结果和AI分析
